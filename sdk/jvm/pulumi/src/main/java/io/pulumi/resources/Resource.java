...
public class Resource {
    @Nullable
    //@Output(Constants.UrnPropertyName)] // TODO C# used an annotation to set this with reflection!
    private /* final */ Output<String> urn; // this can be set only once with the setter
    ...
    protected Resource(
        ...
        this.urn = null; // this.urn can be set later with the setter
        ...
        var parent = type.equals(Stack.InternalRootPulumiStackTypeName)
                ? null
                : (options.parent == null ? Deployment.getInstance().getInternal().getStack() : options.parent);
        ...
        if (options.parent != null) {
            ...
            for (var parentAlias : options.parent.aliases) {
                options.aliases.add(
                        Urn.internalInheritedChildAlias(name, options.parent.getName(), parentAlias, type).toInput()
                );
            }
        ...
        if (custom) {
            ...
            if (provider == null) {
                if (options.parent != null) {
                    ...
                    options.provider = options.parent.internalGetProvider(type);
                }
            } else {
        ...
        Deployment.getInstance().getInternal().readOrRegisterResource(this, remote, DependencyResource::new, args, options);
    }
     ...
     */
    @Nullable
    public ProviderResource internalGetProvider(String moduleMember) {
        var memComponents = moduleMember.split(":");
        if (memComponents.length != 3) {
            return null;
        }

        return this.providers.getOrDefault(memComponents[0], null);
    }
    ...
    private static ImmutableMap<String, ProviderResource> convertToProvidersMap(@Nullable List<ProviderResource> providers) {
        ...
        if (providers != null) {
            for (var provider : providers) {
                result.put(provider.internalGetPackage(), provider);
            }
        }
