...
func (g *generator) lowerExpression(expr model.Expression, typ model.Type) model.Expression {
	...
	expr, diags := hcl2.RewriteApplies(expr, nameInfo(0), !g.futureInit)
	...
	default:
		...
		g.Indented(func() {
			for i, p := range expr.Signature.Parameters {
				g.Fgenf(w, "%s%s %s = values.Item%d;\n", g.Indent, p.Type, p.Name, i+1)
			}
			g.Fgenf(w, "%sreturn %v;\n", g.Indent, expr.Body)
		})
...
func (g *generator) genRange(w io.Writer, call *model.FunctionCallExpression, entries bool) {
	// TODO check equivalent in java
	g.genNYI(w, "Range %.v %.v", call, entries)
}

var functionNamespaces = map[string][]string{
	"readDir":  {"java.nio.file.Files"},
	"readFile": {"java.nio.file.Files"},
...
func (g *generator) genFunctionUsings(x *model.FunctionCallExpression) []string {
	...
	return []string{fmt.Sprintf("io.pulumi.%[1]s", pkg)}
}
...
func (g *generator) GenFunctionCallExpression(w io.Writer, expr *model.FunctionCallExpression) {
	...
	case hcl2.IntrinsicApply:
		g.genApply(w, expr)
	case intrinsicAwait:
		g.Fgenf(w, "%.17v.join();", expr.Args[0])
	...
	case "fileArchive":
		g.Fgenf(w, "new asset.FileArchive(%.v)", expr.Args[0])
	case "fileAsset":
		g.Fgenf(w, "new asset.FileAsset(%.v)", expr.Args[0])
	case hcl2.Invoke:
		...
		g.Fgenf(w, "%s(%.v%v).join()", name, expr.Args[1], optionsBag)
	case "length":
	...
	case "readFile":
		g.Fgenf(w, "Files.readAllLines(Paths.get(%v), StandardCharsets.UTF_8)", expr.Args[0])
	case "readDir":
		// TODO check equivalent in java
...
func (g *generator) genDictionary(w io.Writer, expr model.Expression) {
	switch expr := expr.(type) {
	case *model.ObjectConsExpression:
		g.Fgen(w, "new HashMap<String, Optional<Object>>\n")
		g.Fgenf(w, "%s{\n", g.Indent)
		g.Indented(func() {
...
func (g *generator) genObjectConsExpression(w io.Writer, expr *model.ObjectConsExpression, destType model.Type) {
	...
	if typeName != "" {
		...
		g.Indented(func() {
			...
			for _, item := range expr.Items {
				buffer += fmt.Sprintf("%s", g.Indent)
				buffer += fmt.Sprintf(" %.v%s", item.Value, newLineIndicator)
			}

			buffer = strings.TrimRight(buffer, newLineIndicator)
			buffer += "\n"
			g.Fgen(w, buffer)
		})
		g.Fgenf(w, "%s)", g.Indent)
